...
```mermaid
graph TD
    A["Phase 1: Foundation & 1st Syllabus Coverage<br/>(Sep 21, 2026 – Dec 27, 2026 | 14 Weeks / 98 Days)"] --> B["Phase 2: Consolidation & Revision Round 1<br/>(Dec 28, 2026 – Jan 10, 2027 | 2 Weeks / 14 Days)"]
    B --> C["Phase 3: Full-Length Mocks & Revision Round 2<br/>(Jan 11, 2027 – Jan 31, 2027 | 3 Weeks / 21 Days)"]
    C --> D["Phase 4: Final 7-Day Exam Climax<br/>(Feb 1, 2027 – Exam Day Feb 2027 | 1 Week / 7 Days)"]
```
...
```mermaid
graph LR
    subgraph Track_A["Track A: Mathematical & Algorithmic Foundation"]
        DM[Discrete Mathematics] --> LA[Linear Algebra]
        CProg[C Programming] --> DS[Data Structures] --> Algo[Algorithms]
        Prob[Probability & Stats] --> Calc[Calculus]
    end

    subgraph Track_B["Track B: Systems, Theory & Architecture"]
        DL[Digital Logic] --> COA[Computer Org & Arch]
        TOC[Theory of Computation] --> CD[Compiler Design]
        COA --> OS[Operating Systems]
        OS --> DBMS[Databases]
        DBMS --> CN[Computer Networks]
    end
```
...
```mermaid
graph TD
    subgraph Round1["Round 1: Speed & Guaranteed Marks (0 to 75 mins)"]
        R1A["Solve only 100% obvious questions<br/>(1-mark concepts, direct formula, easy apti)"]
        R1B["Skip any question that requires long calculation or deep thought"]
        R1C["Target: Secure 35-40 clean marks, zero negative marks"]
    end
    subgraph Round2["Round 2: Conceptual & Numerical Solving (75 to 145 mins)"]
        R2A["Solve moderate questions marked for review<br/>(DP trace, pipeline latency, multi-level paging)"]
        R2B["Target: Accumulate +25-30 marks"]
    end
    subgraph Round3["Round 3: Tough Nuts & Verification (145 to 180 mins)"]
        R3A["Attack hardest MSQs and deep proof questions"]
        R3B["Double check all NAT decimal bounds & unit conversions"]
    end
